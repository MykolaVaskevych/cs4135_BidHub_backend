...
import com.bidhub.payment.messaging.EventEnvelope;
...
class EventOutboxTest {
    ...
    void enqueue_persistsValidEnvelopeJson() throws Exception {
        ...
        OutboxEvent saved = outbox.enqueue("AuctionSold", "auction.sold", payload);

        assertThat(saved.getStatus()).isEqualTo(OutboxStatus.PENDING);
        assertThat(saved.getEventType()).isEqualTo("AuctionSold");
        assertThat(saved.getRoutingKey()).isEqualTo("auction.sold");
        assertThat(saved.getAttempts()).isZero();
        assertThat(saved.getCreatedAt()).isNotNull();

        EventEnvelope envelope =
                objectMapper.readValue(saved.getEnvelopeJson(), EventEnvelope.class);
        assertThat(envelope.eventType()).isEqualTo("AuctionSold");
        assertThat(envelope.eventId()).isNotNull();
        assertThat(envelope.version()).isEqualTo(1);

        JsonNode body = envelope.payload();
        assertThat(body.get("auctionId").asText()).isEqualTo(auctionId);
        assertThat(body.get("currency").asText()).isEqualTo("EUR");
    }

    @Test
    @DisplayName("enqueue() with null payload still produces valid envelope JSON (payload becomes JSON null node)")
