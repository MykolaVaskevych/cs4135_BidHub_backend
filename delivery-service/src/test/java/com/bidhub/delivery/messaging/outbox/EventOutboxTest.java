...
import com.bidhub.delivery.messaging.EventEnvelope;
...
class EventOutboxTest {
    ...
    void enqueue_persistsValidEnvelopeJson() throws Exception {
        ...
        String deliveryJobId = UUID.randomUUID().toString();
        Object payload =
                Map.of(
                        "deliveryJobId", deliveryJobId,
                        "currency", "EUR");

        OutboxEvent saved = outbox.enqueue("DeliveryJobCreated", "delivery.job-created", payload);

        assertThat(saved.getStatus()).isEqualTo(OutboxStatus.PENDING);
        assertThat(saved.getEventType()).isEqualTo("DeliveryJobCreated");
        assertThat(saved.getRoutingKey()).isEqualTo("delivery.job-created");
        assertThat(saved.getAttempts()).isZero();
        assertThat(saved.getCreatedAt()).isNotNull();

        EventEnvelope envelope =
                objectMapper.readValue(saved.getEnvelopeJson(), EventEnvelope.class);
        assertThat(envelope.eventType()).isEqualTo("DeliveryJobCreated");
        assertThat(envelope.eventId()).isNotNull();
        assertThat(envelope.version()).isEqualTo(1);

        JsonNode body = envelope.payload();
        assertThat(body.get("deliveryJobId").asText()).isEqualTo(deliveryJobId);
