...
class EventEnvelopeJsonRoundTripTest {
    ...
    void roundTrip_preservesAllFields() throws Exception {
        ...
        payload.put("deliveryJobId", UUID.randomUUID().toString());
        ...
        assertThat(decoded.eventType()).isEqualTo("DeliveryJobCreated");
        assertThat(decoded.occurredAt()).isEqualTo(Instant.parse("2026-05-04T10:00:00Z"));
        assertThat(decoded.version()).isEqualTo(1);
        assertThat(decoded.payload().get("deliveryJobId").asText())
                .isEqualTo(payload.get("deliveryJobId").asText());
